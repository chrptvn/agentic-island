...
export function registerWorldCharactersCommand(program: Command): void {
  ...
  chars
    ...
    .action((opts) => {
      ...
      worldRequest<Record<string, CharacterEntry>>(config, "GET", "/api/characters").then(
        (characters) => {
  ...
  chars
    ...
    .action((id, opts) => {
      ...
      worldRequest<{ message: string }>(config, "POST", path, body).then((res) => {
  ...
  chars
    ...
    .action((id, opts) => {
      const config = resolveWorldConfig(opts);
      worldRequest<{ message: string }>(config, "POST", "/api/despawn", { id }).then((res) => {
        printSuccess(res.message);
      });
    });
}
