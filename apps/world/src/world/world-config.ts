...
export const CONFIG_PATH_WORLD = join(__dirname, "../..", "config", "world.json");
...
let _config: WorldConfig = loadWorldConfig();

function loadWorldConfig(): WorldConfig {
  try {
    const raw = JSON.parse(readFileSync(CONFIG_PATH_WORLD, "utf-8")) as Partial<WorldConfig>;
    return {
      ...DEFAULT_WORLD_CONFIG,
      ...raw,
      energyCosts:    { ...DEFAULT_WORLD_CONFIG.energyCosts,    ...(raw.energyCosts    ?? {}) },
      characterStats: { ...DEFAULT_WORLD_CONFIG.characterStats, ...(raw.characterStats ?? {}) },
      equipmentSlots: raw.equipmentSlots ?? DEFAULT_WORLD_CONFIG.equipmentSlots,
      mapGen:         { ...DEFAULT_WORLD_CONFIG.mapGen,         ...(raw.mapGen         ?? {}) },
    };
  } catch {
...
export function reloadWorldConfig(): void {
  _config = loadWorldConfig();
}
