...
export class World extends EventEmitter {
  ...
  syncFromDb(): boolean {
    ...
    if (seedChanged || overridesChanged) {
      if (seedChanged) this.map = new WorldMap(saved!);
      this.overrides = loadOverrides();
  ...
  regenerateMap(options?: MapOptions): WorldMap {
    this.map = new WorldMap(options);
    clearOverrides();
    clearEntityStats();
  ...
  private _checkEnergy(character: CharacterInstance, action: "harvest" | "build" | "interact" | "craft" | "eat" | "moveStep" | "plow"): void {
    const cost = getWorldConfig().energyCosts[action];
  ...
  watchConfigs(): void {
    const configs: Array<{ path: () => string; reload: () => void; name: string }> = [
      ...
      { path: () => CONFIG_PATH_WORLD, reload: reloadWorldConfig, name: "world.json"   },
    ];
