...
export class HubConnector {
  ...
  sendStateUpdate(state: WorldState): void {
    ...
    const msg: WorldToHubMessage = { type: "state_update", state };
  ...
  private attemptConnect(
    ...
    ws.on("open", () => {
      ...
      const handshake: WorldToHubMessage = {
        type: "handshake",
    ...
    ws.on("message", (raw: WebSocket.RawData) => {
      let msg: HubToWorldMessage;
      try {
        msg = JSON.parse(raw.toString()) as HubToWorldMessage;
      } catch {
