      ...
      if (t === "water") {
        ...
        if (groundPrefix) {
          const isSameGround = (nx: number, ny: number) => {
            if (nx < 0 || nx >= w || ny < 0 || ny >= h) return false;
            const nb = biomeGrid.get(`${nx},${ny}`);
            if (!nb || biomeGroundTile.get(nb) !== groundPrefix) return false;
            // Same-biome grass OR same-biome lake water (ground extends to lake edge)
            return terrain[ny][nx] === "grass" || lakeGrid.has(`${nx},${ny}`);
          };
